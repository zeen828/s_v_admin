                    <div class="callout callout-info">
                        <h4>提示</h4>
                        <p><a href="http://<?php echo $api_domain;?>/<?php echo $api_uri;?>" target="_blank">JSON</a></p>
...
?>
                    ...
                    $(document).ready(function(){
                        $('.cancel_cache').off('click').on('click', function() {
                            if (confirm('清除cache？')) {
                                $.ajax({
                                    url: 'http://<?php echo $api_domain;?>/<?php echo $api_uri;?>?debug=debug&cache=delete',
                                    type: 'GET',
                                    cache: false,
                                    headers: {
                        ...
                        $('.run_cron').off('click').on('click', function() {
                            if (confirm('即時統計？')) {
                                $.ajax({
                                    url: 'http://<?php echo $cron_domain;?>/<?php echo $cron_uri;?>',
                                    type: 'GET',
                                    cache: false,
                                    headers: {
                                        'VIDOL-API-KEY' : 'sw84sc888kkcg0ogo8cw4swgkswkw048cc48swk8'
