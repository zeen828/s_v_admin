...
class Datatables extends CI_Controller
{
    ...
    function __construct ()
    {
        ...
        // 初始化
        $this->data_view = format_helper_backend_view_data('datatables_content');
        $this->data_view['system']['action'] = 'Datatables';
        $this->data_view['right_countent']['tags']['tag_2'] = array(
                'title' => 'Datatables測試',
                'link' => '/backend/datatables',
                'class' => 'fa-cog'
        );
        // 效能檢查
    ...
    {
        try {
            if ($this->flexi_auth->is_admin() || true) {
                // 變數
                $data_post = array();
                // grocery_CRUD 自產表單
