	...
	public function vote($config_id) {
		try {
			if ($this->flexi_auth->is_privileged ( 'Charts View' ) && ! empty ( $config_id )) {
				...
				$crud->unset_delete ();
				// 清單顯示欄位
				$crud->columns ( 'date_at', 'registered', 'vote', 'new_vote', 'proportion', 'vidol_registered', 'vidol_proportion', 'single_vote', 'total_vote', 'total_registered' );
				// 資料庫欄位文字替換
				$crud->display_as ( 'id', $this->lang->line ( 'fields_pk' ) );
				$crud->display_as ( 'config_id', '設定檔id' );
				$crud->display_as ( 'vote', '當日投票數' );
				$crud->display_as ( 'new_vote', '第一次投票數' );
				$crud->display_as ( 'single_vote', '[累計]不重複投票數' );
				$crud->display_as ( 'total_vote', '[累計]投票數' );
				$crud->display_as ( 'registered', '投票增加註冊數' );
				$crud->display_as ( 'total_registered', '[累計]投票註冊數' );
	...
	public function mrplay_list() {
		try {
			if ($this->flexi_auth->is_privileged('Charts View')) {
				...
				$crud->display_as('v_total_vote', '[累計]投票數');
				$crud->display_as('v_vote_registered', '投票增加註冊數');
				$crud->display_as('v_registered', 'vidol註冊數');
				$crud->display_as('v_total_registered', '[累計]投票註冊數');
