...
class Lotters_winners_list_model extends CI_Model {
	private $table_name = 'lotters_winners_list_tbl';
	private $fields_pk = 'lw_pk';
	...
	public function insert_lotters_winners_list_for_data($data) {
		$this->w_db->insert ( $this->table_name, $data );
		$id = $this->w_db->insert_id ();
		// echo $this->w_db->last_query();
		return $id;
	}
	public function update_lotters_winners_list_for_data($pk, $data) {
	...
	public function get_row_lotters_winners_list_by_pk($select, $pk) {
		if (! empty ( $select )) {
	 ...
	 * @return unknown
	 */
	public function clear_winners_list_by_lw_lc_pk($pk) {
		$this->w_db->set ( 'lw_status', '0' );
		$this->w_db->where ( 'lw_lc_pk ', $pk );
