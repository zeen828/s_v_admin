...
class Dealers_model extends CI_Model {
	...
	public function insert_Dealers_for_data($data) {
		$this->w_db->insert ( $this->table_name, $data );
		$id = $this->w_db->insert_id ();
		// echo $this->w_db->last_query();
		return $id;
	}
	public function update_Dealers_for_data($pk, $data) {
	...
	public function get_row_Dealers_by_pk($select, $pk) {
		if (! empty ( $select )) {
	...
	public function get_Dealers_by_title_status($select, $title) {
		if (! empty ( $select )) {
			$this->r_db->select ( $select );
		}
		if (! empty ( $title )) {
