...
class User_binding_model extends CI_Model {
	private $table_name = 'User_binding_tbl';
	private $fields_pk = 'ub_pk';
	...
	public function insert_User_binding_for_data($data) {
		$this->w_db->insert ( $this->table_name, $data );
		$id = $this->w_db->insert_id ();
		// echo $this->w_db->last_query();
		return $id;
	}
	public function update_User_binding_for_data($pk, $data) {
	...
	public function get_row_User_binding_by_pk($select, $pk) {
		if (! empty ( $select )) {
